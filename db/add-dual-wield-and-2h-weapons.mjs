...
/**
 * Add dual-wield weapons (dual daggers, dual swords) and 2-handed weapons (greataxes, greatswords)
 * for level 20+ progression
 */
...
const weaponTypes = {
  dualDaggers: {
    ...
    baseDmgMin: 12,
    baseDmgMax: 20,
    description: 'A pair of matched daggers for lightning-fast strikes',
    isTwoHanded: 0,
    rarity: 'uncommon',
  },
  dualSwords: {
    ...
    baseDmgMin: 16,
    baseDmgMax: 26,
    description: 'Twin blades for skilled warriors who favor speed and precision',
    isTwoHanded: 0,
    rarity: 'uncommon',
  },
  greatsword: {
...
async function addWeapons() {
  ...
  console.log(weapons.slice(0, 3).map(w => `  ${w.name} (Lvl ${w.required_level}): ${w.damage_min}-${w.damage_max} dmg, ${w.attack_speed}s speed`).join('\n'));
  console.log('  ...\n');
...
async function addCraftingRecipes(weapons) {
  ...
  const recipeGroups = [
    {
      name: 'Dual Daggers',
      description: 'Craft matched pairs of daggers for rapid dual-wielding combat',
    ...
    {
      name: 'Dual Swords',
      description: 'Forge twin blades for warriors who favor speed and finesse',
      profession_type: 'blacksmithing',
      category: 'weapon',
      min_level: 21,
      max_level: 60,
...
async function verify(weapons) {
  ...
  const dualDaggers = weapons.filter(w => w.weapon_type === 'dagger' && w.slot === 'offhand');
  const dualSwords = weapons.filter(w => w.weapon_type === 'sword' && w.slot === 'offhand');
  ...
  console.log(`  Dual Daggers: ${dualDaggers.length}`);
  console.log(`  Dual Swords: ${dualSwords.length}`);
  console.log(`  Greatswords: ${greatswords.length}`);
  console.log(`  Greataxes: ${greataxes.length}`);
  console.log(`  Total: ${weapons.length}\n`);

  // Sample check
  const sampleWeapon = await client.execute({
    sql: 'SELECT * FROM items WHERE name = ? LIMIT 1',
    args: ['Mithril Greatsword'],
  });

  if (sampleWeapon.rows.length > 0) {
    const weapon = sampleWeapon.rows[0];
    console.log('Sample weapon check:');
    ...
    console.log(`  2H: ${weapon.is_two_handed ? 'Yes' : 'No'}`);
    console.log(`  Type: ${weapon.weapon_type}\n`);
  }

  // Check loot tables
  const lootCheck = await client.execute(
    'SELECT COUNT(*) as count FROM mob_loot WHERE item_id IN (SELECT id FROM items WHERE weapon_type IN (\'dagger\', \'sword\', \'greatsword\', \'greataxe\') AND required_level >= 20)'
  );
...
async function main() {
  try {
    console.log('=== Adding Dual-Wield and 2-Handed Weapons ===\n');
